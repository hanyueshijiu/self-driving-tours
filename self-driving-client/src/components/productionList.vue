  ...
  <div class="listcontainer" v-if="$props.type === 'scenery'">
    <div class="listItem" v-for="(item, index) in props.list" :key="index">
      ...
      <div class="description">
        ...
        <div class="feature" v-if="item.evaluate">
          特色：<span>{{ item.evaluate }}</span>
        </div>
        <div class="evaluate">
  ...
  <div class="hotelList" v-if="props.type === 'hotel'">
    ...
    <div class="sortModule">
      <div class="starLevel">
        ...
        <sapn class="selectItem" v-for="item in priceFilterArr" :key="item.value">{{
          item.name
        }}</sapn>
      </div>
      <div class="priceLevel">
        <span class="title">价格</span>
        <sapn class="selectItem" v-for="item in starFilterArr" :key="item.value">{{
          item.name
        }}</sapn>
      </div>
    </div>
    <!-- 酒店列表 -->
    <div class="listBox">
      <div class="hotelItem" v-for="(item,index) in 10" :key="index">
        <img
          src="https://pavo.elongstatic.com/i/mobile220_220/nw_1aSxjZFcobC.jpg"
          alt=""
        />
        <div class="description">
          <div class="title">
            <span class="name">艺龙安悦酒店(上海浦东大道歇浦路地铁站店)</span>
            <span class="star">舒适型</span>
          </div>
          <div class="address">
            地址：<span>浦东金桥地区 | 浦东金桥浦东大道2123号,浦东新区,上海市,上海,中国</span>
          </div>
          <div class="feature">
            特色：<span>位置在江边杨浦大桥</span>
          </div>
          <div class="evaluate">
            <span class="score">
              </span
